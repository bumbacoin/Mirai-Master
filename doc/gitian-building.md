...
*Setup instructions for a gitian build of MRI using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MRI
...
Clone the git repositories for mirai and gitian and then checkout the mirai version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mirai repository
...
To build MRI (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mirai repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mirai/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/MRICRYPTO/MRI
...
./bin/gbuild --commit mirai=${COMMIT} --url mirai=${URL} ../mirai/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mirai=${COMMIT} --url mirai=${URL} ../mirai/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mirai=${COMMIT} --url mirai=${URL} ../mirai/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mirai-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mirai-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mirai-build.assert
```
...
[mirai/gitian.sigs](https://github.com/mirai-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
